...
export class EmployeeService {
    ...
    query(req?: any): Observable<EntityArrayResponseType> {
        ...
        console.log('paramters PAGINATION ' + options);
        return this.http
    ...
    protected convertDateFromClient(employee: IEmployee): IEmployee {
        const copy: IEmployee = Object.assign({}, employee, {
            dob: employee.dob != null && employee.dob.isValid() ? employee.dob.format(DATE_FORMAT) : null
        });
        return copy;
    }
    ...
    protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
        if (res.body) {
            res.body.forEach((employee: IEmployee) => {
                employee.dob = employee.dob != null ? moment(employee.dob) : null;
            });
        }
        return res;
    }
